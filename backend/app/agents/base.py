...
class Agent:
    ...
    def system_prompt(self, context_text: str, project_instructions: str | None = None) -> str:
        ...
        return (
            ...
            "assignment's status (started/submitted/missing/etc.), "
            "resyncing a document from its live source, generating a "
            "grounded practice quiz or exam-style practice test on a topic "
            "or unit, generating flashcards from a document or unit, or "
            "deleting an assignment/document/calendar event -- using the "
            "tools provided, whenever they clearly ask for one of those. "
            "Actually call the matching tool; don't just describe what you "
            "would do. When you generate a practice quiz/test, show the "
