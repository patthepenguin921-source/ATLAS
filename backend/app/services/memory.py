...
async def semantic_search(
    ...
    threshold: float = 0.0, folder_id: str | None = None,
) -> list[dict[str, Any]]:
...
async def upcoming_assignments(
    user_id: str, *, days: int = 14, limit: int = 25, course_id: str | None = None,
) -> list[dict]:
...
async def overdue_or_missing(
    user_id: str, *, limit: int = 25, course_id: str | None = None,
) -> list[dict]:
    now = datetime.now(timezone.utc)
    filters = {
        "user_id": eq(user_id),
...
async def recent_grades(
    user_id: str, *, limit: int = 15, course_id: str | None = None,
) -> list[dict]:
    filters = {"user_id": eq(user_id)}
    if course_id:
        filters["course_id"] = eq(course_id)
...
async def available_documents(
    user_id: str, *, limit: int = 50, course_id: str | None = None, folder_id: str | None = None,
) -> list[dict]:
...
async def build_context(
    ...
    ``course_id`` (set by the assistant embedded on a course page, see
    `ChatRequest.course_id`) narrows assignments/events/grades/documents to
    just that class and is called out in the rendered context as the
    student's current focus, without dropping the rest of their record
    (courses/facts stay unscoped so cross-class questions still work).
    ``folder_id`` narrows further, to one topic/unit folder within it.
    """
    courses = await courses_overview(user_id)
    ctx: dict[str, Any] = {
        "courses": courses,
        "upcoming": await upcoming_assignments(user_id, course_id=course_id),
        "upcoming_events": await upcoming_events(user_id, course_id=course_id),
        "overdue": await overdue_or_missing(user_id, course_id=course_id),
        "recent_grades": await recent_grades(user_id, limit=10, course_id=course_id),
        "review_due": await concepts_needing_review(user_id, limit=10),
        "repeated_mistakes": await repeated_mistakes(user_id, limit=10),
        "documents": await available_documents(user_id, course_id=course_id, folder_id=folder_id),
        "learned_facts": await learned_facts(user_id),
    }
    if course_id:
        match = next((c for c in courses if c["id"] == course_id), None)
        ctx["focused_course"] = match["name"] if match else None
    if query and include_semantic:
        try:
            ctx["relevant_passages"] = await semantic_search(
                user_id, query, limit=semantic_limit, course_id=course_id, folder_id=folder_id,
            )
...
def render_context(ctx: dict[str, Any]) -> str:
    ...
    if ctx.get("focused_course"):
        lines.append(
            ...
            "This question was asked from that class's own page — assignments, "
            "events, grades, and documents below are scoped to it. Still answer "
            "a question about a different class if the student clearly asks one."
        )
